...
from typing import Any, Callable, Optional, TypeAlias, Union, cast
...
try:
    ...
    from torch.distributed.pipelining._utils import PipeInfo, flatten_args, validate_tensors_metadata
...
_BackwardResult: TypeAlias = tuple[tuple[Optional[torch.Tensor], ...], Optional[list[dict[str, Any]]]]
...
class _PipelineStageBase(ABC):
    ...
    def backward_maybe_with_nosync(
        ...
        def _perform_backward() -> Callable[[], _BackwardResult]:
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                result = _perform_backward()()
            else:
                with self.submod.no_sync():  # type: ignore[operator]
                    result = _perform_backward()()
        # If submod is a FSDP module or regular module
        else:
            result = _perform_backward_with_fsdp(
                root_module=self.submod,
                last_backward=last_backward,
                perform_backward=_perform_backward,
            )
...
def _perform_backward_with_fsdp(
    root_module: torch.nn.Module,
    last_backward: bool,
    perform_backward: Callable[[], Callable[[], _BackwardResult]],
) -> _BackwardResult:
    """
    Executes the backward pass with FSDP enabled. With PP, the weights are unsharded
    (using an all-gather) on the forward pass of the first microbatch. The weight
    gradients are synchronized using a reduce-scatter on the backward pass of the last
    microbatch. To prevent weights from being all-gathered, and weight gradients from
    being reduced on all microbatches, FSDP is effectively turned off on the backward
    pass by setting `is_last_backward`, `reshard_after_backward` and
    `requires_gradient_sync` to False for all microbatches. After all the backward passes
    are complete, the FSDP reduce-scatter is run explicitly by invoking `post_backward`.

    Args:
        root_module (torch.nn.Module): Root nn.Module for the current pipeline stage.
        last_backward (bool): Whether this is the last microbatch.
        perform_backward (Callable[[], Callable[[], _BackwardResult]]): Function
            that returns a callable that can perform the backward pass.

    Return:
        The result after performing the backward pass for the current microbatch.
    """

    def disable_fsdp(module: torch.nn.Module):
        ...
        for submodule in module.children():
            disable_fsdp(submodule)

    disable_fsdp(root_module)

    result = perform_backward()()

    def run_post_backward(module: torch.nn.Module):
        for submodule in module.children():
            run_post_backward(submodule)
        ...
        if isinstance(module, FSDPModule):
            ...
            # It would be much better if pipelining backward invoked `.backward`,
            # so autograd hooks worked and modules like DDP/FSDP behaved as
            # expected. Working around this for the time being, we need to call
            # this too to ensure FSDP syncs its grad reduction ops back to the
            # default stream.
            fsdp_state._root_post_backward_final_callback()

    # Manually call post backward for FSDP after the last microbatch is complete.
    if last_backward:
        run_post_backward(root_module)
...
class _PipelineStage(_PipelineStageBase):
    ...
    def _move_submod_to_device(self):
        ...
        # that case.
        has_meta_param = any(isinstance(p, FakeTensor) or p.is_meta for p in self.submod.parameters())
        if has_meta_param:
...
class PipelineStage(_PipelineStageBase):
    ...
    def __init__(
        ...
        if input_args is None:
            ...
            self.inputs_meta = (input_args,) if isinstance(input_args, torch.Tensor) else input_args
