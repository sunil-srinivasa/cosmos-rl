...
class vLLMRolloutWorker(RolloutWorkerBase):
    ...
    def recv_weight_shard(
        ...
        target_dtype = str2torch_dtype(self.config.train.param_dtype)
        ...
        for insts_for_per_param in insts_group.param_instructions:
            ...
            for inst in insts:
                ...
                recv_tensor = None
                no_need_buffer = vllm_tensor_view.is_contiguous() and vllm_tensor_view.dtype == target_dtype
                if no_need_buffer:
                    recv_tensor = vllm_tensor_view
                else:
                    # new a temp tensor
                    recv_tensor = torch.empty_like(vllm_tensor_view).contiguous().to(target_dtype)
                logger.debug(
                ...
                if "model.embed_tokens.weight" in inst_dest_name:
                    logger.info(f"======= recv {recv_tensor.shape}, {recv_tensor.dtype}")
        ...
        def completion_lambda(all_cloned_target_tensors, tensors_to_check):
            ...
            for (
                ...
                cloned_target_tensor = cloned_target_tensor.to(target_dtype).to(cloned_target_tensor.dtype)
