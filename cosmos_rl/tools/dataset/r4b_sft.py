...
def pad_for_batching(
    pixel_values: list[torch.Tensor],
):
    if len(pixel_values) == 1:
        return pixel_values

    def pad_to_max_patch(pixel_value: torch.Tensor, max_patch: int, pad_value=0.0):
        cur_patch = pixel_value.shape[1]
        if cur_patch == max_patch:
            return pixel_value
        pad_shape = list(pixel_value.shape)
        pad_shape[1] = max_patch - cur_patch
        pad_tensor = torch.full(
            pad_shape, pad_value, dtype=pixel_value.dtype, device=pixel_value.device
        )
        return torch.cat([pixel_value, pad_tensor], dim=1)

    max_patch = max(x.shape[1] for x in pixel_values)
    pixel_values = torch.cat(
        [pad_to_max_patch(image, max_patch) for image in pixel_values], dim=0
    )
    return pixel_values


class R4BDataPacker(DataPacker):
    """
    Data protocol & processing logic for R-4B SFT.
    """
    ...
    def _process_single_sample(
        self,
        conversation: "R4BDataPacker.Payload",
    ...
    def _collate_fn(
        ...
        if all([x is not None for x in pixel_values]):
            pixel_values = pad_for_batching(pixel_values)
        else:
    ...
    def sft_process_sample(self, sample: "R4BDataPacker.Payload") -> Dict[str, Any]:
        """
